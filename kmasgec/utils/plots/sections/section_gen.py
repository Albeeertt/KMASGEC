...
class Gen(Section):
    ...
    def define_section(self, logits : np.array, labels: np.array, x: int, y: int):
        ...
        gene_clean: np.array = logits[mask_clean_gene]
        annotation_gene_clean: np.array = labels[mask_clean_gene]

        gene_half: np.array = logits[mask_dirty_gene]
        annotation_gene_half: np.array = labels[mask_dirty_gene]

        gene_bad: np.array = logits[mask_bad_gene]
        ...
        metrics_logLoss_good: float = super().cross_entropy(gene_clean, annotation_gene_clean)
        metrics_logLoss_half: float = super().cross_entropy(gene_half, annotation_gene_half)
        metrics_logLoss_bad: float = super().cross_entropy(gene_bad, annotation_gene_bad)
        ...
        x_obj, y_obj, width_obj, height_obj = x_obj, (y_obj+height_obj), width_obj, height_obj
        html += super().add_text(f"Entropía cruzada sobre los genes sin región intergénica: {metrics_logLoss_good}.", x_obj, y_obj, width_obj, height_obj)
        x_obj, y_obj, width_obj, height_obj = x_obj, (y_obj+height_obj), width_obj, height_obj
        html += super().add_text(f"Entropía cruzada sobre los genes con región intergénica: {metrics_logLoss_half}.", x_obj, y_obj, width_obj, height_obj)
        x_obj, y_obj, width_obj, height_obj = x_obj, (y_obj+height_obj), width_obj, height_obj
        html += super().add_text(f"Entropía cruzada sobre los genes mal anotados: {metrics_logLoss_bad}.", x_obj, y_obj, width_obj, height_obj)
