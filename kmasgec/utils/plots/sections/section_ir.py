...
class IntergenicRegion(Section):
    ...
    def define_section(self, logits: np.array, labels: np.array, x: int, y: int):
        ...
        ir_clean: np.array = logits[mask_clean_ir]
        annotation_ir_clean: np.array = labels[mask_clean_ir]

        ir_half: np.array = logits[mask_dirty_ir]
        annotation_ir_half: np.array = labels[mask_dirty_ir]

        ir_bad: np.array = logits[mask_bad_ir]
        ...
        metrics_f1 = super().f1_metric(logits, labels)

        metrics_logLoss_good: float = super().cross_entropy(ir_clean, annotation_ir_clean)
        metrics_logLoss_half: float = super().cross_entropy(ir_half, annotation_ir_half)
        metrics_logLoss_bad: float = super().cross_entropy(ir_bad, annotation_ir_bad)

        html: str = ""
        # html += super().create_section(x, y)
        x_obj, y_obj, width_obj, height_obj = x, y, 120, 120
        ...
        x_obj, y_obj, width_obj, height_obj = x_obj, (y_obj+height_obj), width_obj, height_obj
        html += super().add_text(f"Entropía cruzada sobre las regiones intergénicas sin genes: {metrics_logLoss_good}.", x_obj, y_obj, width_obj, height_obj)
        x_obj, y_obj, width_obj, height_obj = x_obj, (y_obj+height_obj), width_obj, height_obj
        html += super().add_text(f"Entropía cruzada sobre las regiones intergénicas con genes: {metrics_logLoss_half}.", x_obj, y_obj, width_obj, height_obj)
        x_obj, y_obj, width_obj, height_obj = x_obj, (y_obj+height_obj), width_obj, height_obj
        html += super().add_text(f"Entropía cruzada sobre las regiones intergénicas mal anotadas: {metrics_logLoss_bad}.", x_obj, y_obj, width_obj, height_obj)
